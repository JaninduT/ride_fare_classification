...
tree_model=tree_model.fit(train_data,train_label)

predictions = tree_model.predict(predict_data)

output = pd.DataFrame({'tripid': predict_index, 'prediction': predictions})
output.to_csv('sample_submission_decision_tree_depth_6.csv', index=False)
